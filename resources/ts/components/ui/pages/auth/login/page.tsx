...
import z from "zod";
import { loginInSchema } from "../../../../../schema/auth/auth";
...
export default function LoginPage() {
    ...
    const onSubmit = async (values: z.infer<typeof loginInSchema>) => {
        ...
        startTransition(async () => {
            //   const data = await // should be API call here to handle login like fitch("/api/auth/login" ...
            //   if (data?.error) {
            //     form.reset();
            //     setError(data.error);
            //     return;
            //   }
            //   if (data?.success) {
            //     router.push(data.redirectUrl); route there not work wiht laravel
            //   }
        });
    };
            ...
            >
                <MagicCard gradientColor="rgba(0,0,0,0.95)" className="p-0 ">
                    <CardHeader className="border-b border-white/20 p-4">
                        <CardTitle className="text-white text-center mt-4">
                            Login
                        </CardTitle>
